// ================================================================================
// Name        : gather_v0.cpp   (v0: 8 cofilters / diversified fingerprint moduli)
...
//
// Hash Diversification Versions:
//  _v0: hashes with common base but with individual small moduli
//  _v1: hashes with common small modulus but with individual bases
//  _v2: hashes with common base and common small modulus but obtained
//       from individually shuffled shingles
// where base and modulus are the parameters of the Rabin fingerprint and
// 'small' means small with respect to the size of the cache-lines
//
// Demo-String:
...
// input: read hash map
const string map_file_name_prefix=    "C:\\cr\\map_";
...
#define NS      1000000000ULL		// length of the test string S (NS bytes)
...
#define B_DIV   257					// base of the diversified hashes

// gather_v0: diversified fingerprint moduli
// -----------------------------------------
// 1 cache-line	 ( 64 bytes)
const uint8_t M_DIV[DV]= {31, 37, 41, 43, 47, 53, 59, 61};
// 2 cache-lines (128 bytes)
// const uint8_t M_DIV[DV]= {89, 97, 101, 103, 107, 109, 113, 127};
// 4 cache-lines (256 bytes)
// const uint8_t M_DIV[DV]= {211, 223, 227, 229, 233, 239, 241, 251};
// biggest modulus
#define M_DIV_MAX  M_DIV[7]
...
const uint16_t C_DIV[DV]= {c_div(0), c_div(1), c_div(2), c_div(3),
...
int main() {
	...
	// L compatible map file (same shingle length in scatter and gather!!)
	string map_file_name= map_file_name_prefix + to_string(L) + ".txt";
	...
	printf("max diversity modulus : %d \n", M_DIV_MAX);
	...
	// hash map allocation
	// -------------------
	map= (uint8_t *)malloc(M_COM + M_DIV_MAX); if (map == NULL) exit(11);
...
inline void update_div_hashes(uint8_t hash[], uint8_t hash1[], const uint8_t s[]) {
	for (uint8_t id= 0; id < DV; id++) {
		hash1[id]= hash[id]= (256*M_DIV[id] + s[L]  +  hash[id] * B_DIV  -  C_DIV[id] * s[0]) % M_DIV[id];
	}
}
	...
	uint8_t  div_hash[]) 	// output: batch of (hash_count * DV) diversified hashes
{
	...
	for (uint8_t id= 0; id < DV; id++) {
		...
		for (uint32_t j= 0; j < L; j++) {
			div_hash[id]= (div_hash[id] * B_DIV + s[j]) % M_DIV[id];
		}
...
time_t load_hash_map(string map_file_name) {
	...
	if (map_length < M_COM+M_DIV_MAX) {
		printf("hash map file length < M_COM+M_DIV_MAX : %llu, %d \n", M_COM, M_DIV_MAX);
		fflush(stdout);
		exit(27);
	}
	...
	// read hash map
	map_input_stream.read((char *)map, M_COM+M_DIV_MAX);
